...
/**
 * COMP557 - Mesh simplification application Elias Al Homsi 260797449
 */
public class MeshSimplificationApp implements SceneGraphNode, Interactor {
	...
	private String[] soupFiles = { "meshdata/tetrahedron.obj", "meshdata/topologytest.obj",
			"meshdata/ico-sphere-tris.obj", "meshdata/icosphere2.obj", "meshdata/cube.obj", "meshdata/cube2obj.obj",
			"meshdata/bunny.obj", "meshdata/cow.obj", "meshdata/monkey.obj", };
	...
	public MeshSimplificationApp() {
		...
		EasyViewer ev = new EasyViewer("Comp 557 Mesh Simplification - Ed AL Homsi 260797449", this,
				new Dimension(400, 400), new Dimension(600, 650));
	...
	private void loadSoupBuildAndSubdivide(String filename) {
		...
		if (heds.faces.size() > 0) {
			currentHE = heds.getBestCandidate(); // set the best candidate
		}
	}
	...
	public void display(GLAutoDrawable drawable) {
		...
		if (scaleWithLOD.getValue()) {
			...
			s = s * s; // or could do Math.pow( s, factor ) to adjust how the scaling goes with LOD,
						// but quadratic seems good
			gl.glScaled(s, s, s);
		}
		...
		HalfEdge.drawError = drawEdgeErrors.getValue();
		...
		EasyViewer.printTextLines(drawable, soupFiles[whichSoup] + " Faces = " + heds.faces.size(), 10, 20, 15,
				GLUT.BITMAP_8_BY_13);
	...
	public JPanel getControls() {
		...
		JTextArea ta = new JTextArea("   space - half edge twin \n" + "   n - half edge next \n"
				+ "   c - collapse current half edge \n" + "   C - collapse in loop until no more to do \n"
				+ "   m - collapse best half edge \n" + "   M - collapse best in loop until no more to do \n"
		...
		component.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_SPACE) {
					...
					if (whichSoup < soupFiles.length - 1)
						whichSoup++;
					loadSoupBuildAndSubdivide(soupFiles[whichSoup]);
				} else if (e.getKeyCode() == KeyEvent.VK_C) {
					if (e.isShiftDown()) {
						while (!heds.topologicalProblem(currentHE)) {
							currentHE = heds.normalcollapse(currentHE, regularizationWeight.getValue());
						}
					} else {
						currentHE = heds.normalcollapse(currentHE, regularizationWeight.getValue());
					}
				} else if (e.getKeyCode() == KeyEvent.VK_G) {
					currentHE = heds.getBestCandidate();
					...
					if (e.isShiftDown()) {
						while (currentHE != null) {
							currentHE = heds.collapse(regularizationWeight.getValue());
						}
					} else {
						currentHE = heds.collapse(regularizationWeight.getValue());
					}
