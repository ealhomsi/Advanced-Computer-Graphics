...
/**
 * A factory class to generate ray tracer objects from XML definition. 
 */
public class Parser {
	...
	public static Scene createScene(Node dataNode) {
        ...
        for ( int i = 0; i < nodeList.getLength(); i++ ) {
            ...
            if ( nodeName.equalsIgnoreCase( "material" ) ) {                
            	...
            	MetaBalls meta = Parser.createMetaBalls(n);
            	scene.surfaceList.add( meta );
            }
        }
        return scene;
	}
	
	private static MetaBalls createMetaBalls(Node dataNode) {
		MetaBalls meta = new MetaBalls();
		ArrayList<Point3d> list = new ArrayList<Point3d>();
		Node centersAttr = dataNode.getAttributes().getNamedItem("centers");
		if ( centersAttr != null ) {
            Scanner s = new Scanner( centersAttr.getNodeValue() );
		...
		if ( radiusAttr != null ) {
			 meta.radius = Double.parseDouble( radiusAttr.getNodeValue() );
		}
		meta.material = parseMaterial(dataNode, "material");	
    	return meta;
	}
	...
	public static Render createRender(Node dataNode) {
		...
		if ( jitterAttr != null) {
			String attribute = jitterAttr.getNodeValue();
			if (attribute.equals("true")) render.jitter = true;
		}

		Node depthAttr = dataNode.getAttributes().getNamedItem("depthOfField");
		if ( depthAttr != null) {
			String attribute = depthAttr.getNodeValue();
			if (attribute.equals("true")) render.depthofField = true;
		}
		
		Node threadAttr = dataNode.getAttributes().getNamedItem("threads");
		if ( threadAttr != null) {
			String attribute = threadAttr.getNodeValue();
			render.threads = Integer.parseInt(attribute);
		}
    	NodeList nodeList = dataNode.getChildNodes();
    	for (int i = 0; i < nodeList.getLength(); i++) {
    		Node n = nodeList.item(i);
